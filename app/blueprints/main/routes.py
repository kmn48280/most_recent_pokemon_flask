    ...
    if request.method == 'POST' and form.validate_on_submit():
        ...
        if response.ok:
            ...
            if poke_name != Pokemon.query.filter_by(name=poke_name):
                ...
                return render_template('pokedex.html.j2', poke = poke_dict, form = form)
            else:
...
@main.route('/add_to_pokemon_bank/<int:id>', methods=['GET', 'POST'])
@login_required
def add_to_pokemon_bank():
    pokemon = Pokemon.query.get(id)
    for pokemon in current_user.pokemon_bank:
        if pokemon == current_user.pokemon_bank[pokemon.id]:
            flash(f'You already have {{pokemon.name}} in your collection', 'danger')
            return redirect(url_for('main.pokedex.html.j2'))
        else:
            current_user.add_pokemon(id)
            current_user.current_poke()
    return render_template('pokedex.html.j2')

@main.route('/del_pokemon_from_bank/<int:id>', methods = ['GET','POST'])
@login_required
def del_pokemon_from_bank():
    pass
